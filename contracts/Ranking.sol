...
/**
 * @notice Linked list which requires entries being linked in points descending order, from head to tail
...
contract Ranking is Controlled {
    ...
    function include(bytes32 _uid, uint256 _points, bytes32 _before) external onlyController {
        ...
        entries[_uid] = Entry(block.timestamp, _points, next, _before);
